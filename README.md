...
fact CRUD), `extraction.py` (fact extraction), `retrieval.py`
(relevant-memory retrieval), `decay.py` (contradiction/update handling),
`embeddings.py` (embed / serialize helpers), `config.py` (env loading,
model names, tunables), `oncemore` (shell launcher).

## Architecture

**Storage: hybrid structured + embeddings, in one SQLite file.** The brief
leaves this open — "structured facts, embeddings, hybrid — your choice,
with reasoning" — so here's the reasoning: pure structured storage can't
do fuzzy semantic recall ("what's going on with their ex" needs to match a
fact stored under a different phrasing). Pure-vector storage can't
reliably answer "is this new fact about the same thing as an old one" for
contradiction detection, which needs the subject/predicate fields to line
up structurally, not just semantically. SQLite specifically because the
only hard persistence requirement is "survive a process restart" — a
single file, zero infra to stand up.

**Two tables**, my own design (the brief specifies the requirement, not a
schema):
...
brief's persistence requirement ("persist across sessions... this is not
just conversation history in the context window") is specifically about
*facts* surviving a restart, not the raw transcript. Persisting the full
history too would be extra infra for a requirement that isn't there, and
summarizing/compressing it would be scope beyond the core loop — my own
scope call, not something the brief explicitly asks for either way, but
flagged here rather than left implicit.

**Extraction (step 2)**: after each user turn (not the assistant's reply),
a single `gpt-4.1-mini` call with a strict JSON-schema response decides
whether the message contains a memory-worthy fact and, if so, extracts
`subject` / `predicate` / `value` in one round trip. The brief only asks to
"define what counts as a memory-worthy fact, how it's extracted... and
where/how it's stored" — it doesn't prescribe a classify-then-extract
split, so one call covers both, my own design choice for simplicity.
`subject` defaults to `"user"` but the model can name a third party the
user mentioned (e.g. "Jordan"). `predicate` is free-text snake_case rather
than a fixed enum — an enum would require anticipating every fact category
up front, and the contradiction-check step (step 4) matches semantically
anyway, so canonical predicates aren't load-bearing. Every extracted fact
is embedded (`text-embedding-3-small`, over `"{predicate}: {value}"`) and
inserted as `active` immediately — no contradiction check yet, that's step
4. Extraction failures (bad JSON, API error) are swallowed and treated as
"no fact" — it's a side effect of the turn, never allowed to break the
conversation.
...
(step 5, and the eval table below) can verify what got captured without
querying the DB directly. It's tagged distinctly from the companion's own
dialogue so it doesn't read as in-character.

**Retrieval (step 3)**, addressing the brief's "retrieve relevantly...
not dump everything into context, and not miss something clearly
relevant": before the persona call, the user's message is embedded and
compared via cosine similarity (numpy) against every *active* fact's
stored embedding — no vector DB, no ANN index, a linear scan over the fact
table, a reasonable tradeoff at this scale (single user, a session's worth
of facts) since the brief leaves the implementation open. Top-k (k=5, my
own default, not brief-specified) go into a second, separate system
message injected on every call, alongside the (always-present) persona
block and the recent-turn window — my approach to the brief's "stay in
character... shouldn't flatten under topic pressure" requirement: never
rely on the model to remember the persona or facts from earlier context,
resend both in full on every call. No similarity floor on retrieval: the
brief's ask is "recall the right memories," and with few facts stored
early in a conversation, everything currently known is reasonably "the
most relevant k" by definition.

**Update/decay (step 4)** — the judgment-heavy piece. For each
...
Addressing the brief's "stay in character... over 50+ turns" requirement.
...
- **A job promotion didn't retire the original job fact.** Verified
  ...
  older one. This is exactly the kind of ambiguous case the brief's
  "update and decay" requirement warns about ("old fact should be updated
  or retired, not just added alongside"), and it's now in the eval table
  below rather than silently accepted.

## What was cut

The brief's own out-of-scope list (§4): UI/UX polish beyond a terminal
loop, auth/billing/multi-user support, voice/image/video generation,
production-scale infra. Its eval-harness section (§3) frames a full
automated harness and an oracle baseline as optional stretch goals, not
requirements — built a scoped-down version instead (see below) given time
constraints and the brief's own priority ordering. Memory
summarization/compression beyond the core loop isn't explicitly addressed
by the brief either way — my own scope call, not a stated non-goal. Also
cut, specific to this build:
...
brief and held up through testing without needing a rewrite. The one real
...
- The 0.75 cosine-similarity threshold for the contradiction candidate
  fallback is a reasonable but uncalibrated constant — not tuned against a
  labeled set of same-topic vs. different-topic fact pairs.
...
- "Does this update the old fact, or just refine/add detail to it" is
  genuinely ambiguous in cases like a job promotion (see the manual test
  above) — the contradiction checker treated it as non-contradictory,
  leaving two active `job` facts rather than one superseding the other.
  Reasonable call, but it means predicate-level fact fragmentation is
  possible over a long enough conversation, and isn't specifically
  guarded against.
